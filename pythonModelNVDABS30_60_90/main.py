...
<<<<<<< HEAD
                ...
                # 6. Visualization
                print(f"Generating Plots for {ticker} (Saving to PDF)...")
                Visualizer.plot_paths(paths_df, title=f"Monte Carlo Paths: {ticker}", num_paths_to_plot=150, pdf=pdf)
                Visualizer.plot_distribution(paths_df, strike_price=s_price, pdf=pdf)
                Visualizer.plot_greeks(spot_range, greeks_data, title=f"Griegos: {ticker} (Strike={s_price:.2f})", pdf=pdf)
                Visualizer.plot_gex_profile(spot_range, greeks_data['gex'], current_price, title=f"Perfil GEX: {ticker}", pdf=pdf)
...
=======
            ...
            traceback.print_exc()
            continue
>>>>>>> 8f87013fc7512eeb0233a2c149dd25b5724263af
